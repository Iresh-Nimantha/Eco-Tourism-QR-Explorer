...
const HeroSection = () => {
  ...
  useEffect(() => {
    function handleResize() {
      let width =
        cardContainerRef.current?.offsetWidth || window.innerWidth - 180;
  ...
  const handleMobileScroll = () => {
    ...
    const container = sliderScrollRef.current;
    const cardW = container.firstChild
      ...
      <div className="relative z-10 flex w-full flex-col items-center gap-y-12 pt-20 pb-10 md:pt-32 lg:pt-30">
        <div className="flex w-full max-w-2xl flex-col items-center px-4 text-center">
          ...
          <p className="max-w-2xl text-base leading-snug text-white drop-shadow sm:text-lg md:leading-normal">
            Every place has a story. Scan our QR codes at destinations across
            the island to unlock the history, culture, and beauty hidden in
            plain sight.
          </p>
          <button
            className="mt-8 transform rounded-full bg-emerald-600 px-8 py-3 text-lg font-semibold text-white shadow-lg drop-shadow-md transition-all duration-300 hover:scale-105 hover:bg-emerald-700"
