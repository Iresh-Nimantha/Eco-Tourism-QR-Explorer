...
// ShareIcons component with inline SVGs
const ShareIcons = ({
  placeTitle,
  currentUrl,
}: {
...
type Props = {
  place: any;
  relatedPlaces: LocationData[];
  placeId: string;
};

export default function ClientPlaceDetail({
  place,
  relatedPlaces,
  placeId,
}: Props) {
  ...
  if (!place) {
    return (
      <div
        className="min-h-screen flex items-center justify-center p-4 sm:p-6 md:p-10 bg-gradient-to-br from-gray-50 to-green-50/30"
        role="alert"
      >
        <div className="text-center max-w-md mx-auto bg-white rounded-2xl shadow-lg p-6 sm:p-8">
          ...
          <p className="text-gray-600 mb-4 sm:mb-6 text-sm sm:text-base">
            The place ID{" "}
            <span className="font-semibold text-red-500">{placeId}</span> does
      ...
      <article className="pt-[60px] sm:pt-[74px]">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 md:px-8 lg:px-12 py-6 sm:py-8 md:py-12">
          ...
          <header className="mb-8 sm:mb-12">
            {place.imageUrl || place.customFilename ? (
                ...
                <div className="relative">
                  ...
                  place.imageUrl.includes("firebasestorage.googleapis.com") ? (
                    <img
                      ...
                      width={800}
                      height={500}
                      className="w-full h-auto object-contain bg-gray-100 rounded-2xl"
                      loading="eager"
                      draggable={false}
                    />
                  ) : (
                    <img
                      src={place.customFilename}
                      alt={place.locationName || place.name || "Place image"}
                      className="w-full h-auto object-contain bg-gray-100 rounded-2xl"
                      loading="lazy"
                      draggable={false}
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent rounded-2xl flex flex-col justify-end p-4 sm:p-6 md:p-8">
                    <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-2 sm:mb-3 leading-tight drop-shadow-lg">
